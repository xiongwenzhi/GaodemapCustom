...
public class CityPickerDialogFragment extends DialogFragment implements TextWatcher,
    ...
    private void initViews() {
        ...
        choose_city = mContentView.findViewById(R.id.choose_city);
        location_city = mContentView.findViewById(R.id.location_city);
...
//            window.setLayout(width, height - ScreenUtil.getStatusBarHeight(getActivity()));
    ...
    protected void doSearchQuery() {
        ...
        currentPage = 0;
        query = new PoiSearch.Query(mSearchBox.getText().toString().trim(), "", choose_city.getText().toString());// 第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
