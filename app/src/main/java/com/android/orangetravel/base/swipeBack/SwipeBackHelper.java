...
//
//    private static final int SHADOW_WIDTH = 15;// 默认阴影宽度(dp)
//    private static final int EDGE_SIZE = 20;// 默认拦截手势区间(dp)
//
//    private int mShadowWidth;// 阴影宽度(px)
//    private int mEdgeSize;// 拦截手势区间(px)
//    private boolean mIsSliding;// 是否正在滑动
//    private boolean mIsSlideAnimPlaying;// 滑动动画展示过程中
//    private float mDistanceX;// 当前滑动距离(px,正数或0)
...
//     * @param backManager
//     */
//    public SwipeBackHelper(SlideBackManager backManager) {
//        if (null == backManager || null == backManager.getSlideActivity()) {
//            throw new RuntimeException("Neither SlideBackManager nor the method 'getSlideActivity()' can be null!");
//        }
//
//        mActivity = backManager.getSlideActivity();
//        mIsSupportSlideBack = backManager.supportSlideBack();
...
//
//        // 阴影宽度
//        mShadowWidth = DensityUtil.dp2px(SHADOW_WIDTH);
//        // 滑动拦截事件的区域
//        mEdgeSize = DensityUtil.dp2px(EDGE_SIZE);
//    }
//
...
//                sendEmptyMessage(MSG_ACTION_DOWN);/*点击事件*/
//                break;
//            }
...
//                mBundle.putFloat(CURRENT_POINT_X, curPointX);
//                Message message = obtainMessage();
//                message.what = MSG_ACTION_MOVE;/*滑动事件*/
...
//                    return true;
//                } else {
//                    MotionEvent cancelEvent = MotionEvent.obtain(ev);// 首次判定为滑动需要修正事件:手动修改事件为ACTION_CANCEL,并通知底层View
...
//                    mIsSliding = false;
//                    sendEmptyMessage(MSG_ACTION_UP);/*点击结束*/
//                    return false;
//                }
...
//            mAnimatorSet.cancel();
//        }
//        removeMessages(MSG_ACTION_DOWN);/*点击事件*/
//        removeMessages(MSG_ACTION_MOVE);/*滑动事件*/
//        removeMessages(MSG_ACTION_UP);/*点击结束*/
//        removeMessages(MSG_SLIDE_CANCEL);/*开始滑动,不返回前一个页面*/
//        removeMessages(MSG_SLIDE_CANCELED);/*结束滑动,不返回前一个页面*/
//        removeMessages(MSG_SLIDE_PROCEED);/*开始滑动,返回前一个页面*/
//        removeMessages(MSG_SLIDE_FINISHED);/*结束滑动,返回前一个页面*/
//        mActivity = null;
//    }
//
//    @Override
...
//                    sendEmptyMessage(MSG_SLIDE_PROCEED);/*开始滑动,返回前一个页面*/
//                } else {
//                    sendEmptyMessage(MSG_SLIDE_CANCEL);/*开始滑动,不返回前一个页面*/
//                }
//                break;
//            }
...
//        TypedArray a = null;
//        try {
//            a = mActivity.getTheme().obtainStyledAttributes(new int[]{android.R.attr.windowBackground});
//            return a.getColor(0, ContextCompat.getColor(mActivity, android.R.color.transparent));
//        } finally {
//            if (null != a) {
//                a.recycle();
//            }
...
//            sendEmptyMessage(MSG_SLIDE_CANCELED);/*结束滑动,不返回前一个页面*/
//            return;
//        }
//
...
//
//        if (null == previewView || null == currentView) {
//            return;
//        }
//
...
//        float shadowViewEnd = slideCanceled ? mShadowWidth : width + mShadowWidth;
...
//                    sendEmptyMessage(MSG_SLIDE_CANCELED);/*结束滑动,不返回前一个页面*/
//                } else {
//                    sendEmptyMessage(MSG_SLIDE_FINISHED);/*结束滑动,返回前一个页面*/
//                }
//            }
//        });
//        mAnimatorSet.start();
...
//            }
//
//            mPreviousActivity = ActivityLifecycleManage.getInstance().getPreviousActivity();
...
//                mShadowView.setX(-mShadowWidth);
//            }
//            final FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(
//                    mShadowWidth, FrameLayout.LayoutParams.MATCH_PARENT);
